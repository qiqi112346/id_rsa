...
class Graph extends Theme {
  constructor(data, layer, fields, setting, lonlat, options) {
    ...
    if (lonlat) {
      ...
      if (options && options.calGravity) {
        ...
        this.lonlat = [coords[0], coords[1]];
      } else {
        const extent = feature.geometry.extent;
        const coords = extent.center.coordinates;
        this.lonlat = [coords[0], coords[1]];
      }
    }
  ...
  initBaseParameter() {
    ...
    // 数据
    var decimalNumber =
      typeof sets.decimalNumber !== "undefined" && !isNaN(sets.decimalNumber)
        ? sets.decimalNumber
        : -1;
    var dataEffective = Theme.getDataValues(
      this.data,
      this.fields,
      decimalNumber
    );
    ...
    this.DVBParameter = sets.dataViewBoxParameter
    ...
    // 数据视图框中心点
    this.DVBCenterPoint = [
      this.DVBOrigonPoint[0] + this.DVBWidth / 2,
      this.DVBOrigonPoint[1] + this.DVBHeight / 2,
    ];

    // 数据视图框原点与图表框的原点偏移量
    this.origonPointOffset = [
      this.DVBOrigonPoint[0] - op[0],
      this.DVBOrigonPoint[1] - op[1],
    ];
  ...
  resetLocation(lonlat) {
    ...
    this.chartBounds.xmin =  loc[0] - w / 2
    this.chartBounds.ymin = loc[1] - h / 2
    this.chartBounds.xmax = loc[0] + w / 2
    this.chartBounds.ymax = loc[1] + h / 2
    ...
    for (var field in attrs) {
      ...
      try {
        if (!isNaN(decimalNumber) && decimalNumber >= 0) {
          fieldsValue.push(
            parseFloat(attrs[field].toString()).toFixed(decimalNumber)
          );
        } else {
